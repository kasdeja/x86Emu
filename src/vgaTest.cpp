...
namespace
{
    ...
    void WelchWindow(double* window, int size)
    {
        double nm1d2 = (size - 1.0) / 2.0;

        for (int i = 0; i < size; ++i)
            window[i] = 1.0 - (i - nm1d2) / nm1d2;
    }

    void HammingWindow(double* window, int size)
    {
        double CC   = 2.0 * M_PI / (size - 1.0);
        double alfa = 0.53836;
        double beta = 1.0 - alfa;

        for(int i = 0; i < size; i++)
            window[i] = alfa - beta * cos(CC * i);
    }

    void BlackmanWindow(double* window, int size)
    {
        double a0 = 0.42659;
        double a1 = 0.49565;
        double a2 = 0.076849;
...
private:
    ...
    // private methods
    void FillMemoryWithTestContent();
...
Vga::Vga()
{
    ...
    m_videoMem     = reinterpret_cast<uint8_t *>(aligned_alloc(32, 256 * 1024));
    m_videoMemText = reinterpret_cast<uint8_t *>(aligned_alloc(32,  32 * 1024));
...
// private methods
void Vga::FillMemoryWithTestContent()
{
    ...
    for(int n = 0; n < ::strlen(str); n++)
    {
        ...
        m_videoMemText[2 * n + 161] = 15;
    }

    for(int n = 0; n < 256; n++)
        m_videoMemText[2 * n + 160 * 3] = n;

    for(int n = 0; n < 256; n++)
    {
        m_videoMemText[2 * n + 160 * 12] = n;
        m_videoMemText[2 * n + 160 * 12 + 1] =  (1 << 4) + 7;
    }
    ...
    {
        m_videoMem[x] = 7;
        m_videoMem[x + 199 * 320] = 7;
    }

    for(int y = 0; y < 200; y++)
    {
        m_videoMem[y * 320] = 7;
        m_videoMem[y * 320 + 319] = 7;
    }
    ...
    for(int n = 0; n < 256; n++)
    {
        ...
        m_videoMem[addr +   0] = m_videoMem[addr +   1] = m_videoMem[addr +   2] = n;
        m_videoMem[addr + 320] = m_videoMem[addr + 321] = m_videoMem[addr + 322] = n;
        m_videoMem[addr + 640] = m_videoMem[addr + 641] = m_videoMem[addr + 642] = n;
    }

    for(int x = 0; x < 256; x++)
    {
        for(int y = 0; y < 32; y++)
            m_videoMem[x + (y + 32) * 320] = x;
    }
...
Vga::FilterBank Vga::DesignFilter(int inputRate, int outputRate, int taps, double cutoff)
{
    ...
    //HammingWindow(window.data(), flen);
    //BlackmanWindow(window.data(), flen);
...
int main(int argc, char **argv)
{
    ...
    while(running)
    {
        ...
        if (cnt++ < 500)
        {
            vga.DrawTextModeScreenFiltered(reinterpret_cast<uint8_t *>(surface->pixels), surface->w, surface->h, surface->pitch);
        }
        else
        {
            vga.DrawMode13hScreenFiltered(reinterpret_cast<uint8_t *>(surface->pixels), surface->w, surface->h, surface->pitch);
        }
